...
def n_long_words(words, n):
    ...
    list_of_n_long_words = []

    for item in words:
        if len(item) > n:
            list_of_n_long_words.append(item)

    return list_of_n_long_words
...
def halvesies(numbers):
    ...
    list_of_halved_values = []

    for num in numbers:
        num = float(num)/2
        list_of_halved_values.append(num)

    return list_of_halved_values
...
def average(numbers):
    ...
    if len(numbers) != 0:
        ...
        summed_value = float(summed_value)/len(numbers)
...
def join_strings_with_comma(words):
    ...
    if len(words) > 0:
        i = 0
        while i < (len(words) - 1):
            string_with_commas = string_with_commas + words[i] + ", "
            i += 1
        string_with_commas += words[-1]
